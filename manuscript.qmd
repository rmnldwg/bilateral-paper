...
# Results: Prediction of Risk for Occult Disease {#sec-results-risks}
...
- Irradiate LNL II when the tumor extends over the mid-sagittal line
- Irradiate LNL III when LNL II is clinically involved
- Irradiate LNL IV only when the upstream level III is pathologically confirmed via FNA
- Exclude LNLs I and VII for most patients. I.e., only treat these LNLs when they appear clinically metastatic
- Exclude LNL V in all but the most extreme cases. I.e., for advanced T-category tumors with midline extension, where all ipsilateral LNLs and the contralateral LNLs II to IV present with clinical involvement and the upstream involvement is additionally verified by FNA.

In essence, this is a summary of the results in @sec-sec-results-risks. However, it should be considered in light of the limitations outlined in @sec-discussion-limitations below.
...
For some patterns of involvement the difference in the observed prevalence for early vs advanced T-category is greater, for other smaller, than what our model predicts. This is because only one parameter, $p_\text{adv.}$, governs this spread by continuing to evolve the system a certain number of time-steps, while the speed of spread, here the $b_v$ parameters, remains constant. However, it is feasible that these parameters themselves may change over time as the tumor grows into regions with different lymph drainage pathways.


### Sensitivity and Specificity

As hinted at in @sec-data-consensus, the literature values for sensitivity and specificity pairs of the diagnostic modalities do not plausibly match some of the observed data. We therefore assumed the consensus across diagnostic modalities and pathology to represent the ground truth. This simplification allowed us to investigate the models general capability to describe a realistic, if not necessarily entirely accurate dataset. But in the future, this may need to be approached more rigorously, e.g. by differentiating between pathologically confirmed and clinically diagnosed involvement, as the latter is - by definition - incapable of detecting microscopic disease.


### Tumor Subsites

Within the cohort of patients with oropharyngeal tumors, subgroups can be identified whose tumors are located in different subsites, e.g. at the base of the tongue or on the tonsils. The lymphatic metastatic spread patterns of these subgroups may differ slightly. If so, the model could benefit from more fine-grained information about the precise tumor subsite for its training and prediction. We have previously speculated that a mixture model could be a promising approach to capture the subsite specific spread patterns [@zora231470].
