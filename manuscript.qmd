...
This paper builds on the previously developed unilateral model for ipsilateral lymph node involvement presented in [cite 2024 graph extension paper]. Below, we briefly recap the unilateral model to introduce the notation required for extending the framework to a bilateral model, described in @sec-ext-to-contra. For further details on the ipsilateral model, refer to earlier publications [@ludwig_hidden_2021; cite 2024 graph extension paper].
...
We represent a patient’s state of involvement at an abstract time-step $t$ as a vector of hidden binary random variables, where each variable corresponds to a lymph node level (LNL):
...
A hidden Markov model (HMM) is defined by three key components:
