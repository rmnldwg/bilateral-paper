...
At the time of the diagnosis $0 \leq t_D \leq t_\text{max}$, we multiply the result with the observation matrix $\mathbf{B}$ to get the distribution over all possible diagnoses. However, the time of diagnosis $t_D$ is unknown, i.e. we do not know over how many time-steps to evolve the HMM. We solve this problem by marginalizing over the time of diagnosis, possible allowing the diagnosis at any time-step, albeit with different weights. Different distributions over the diagnosis times can then be choosen based on T-category. For instance, the mean of the time-prior to marginalize over the diagnosis time for early T-category patients $P\left( t_D \mid \text{early} \right)$ may be shifted towards earlier times than the one for advanced T-category patients $P\left( t_D \mid \text{early} \right)$. This gives us for example:
