...
\caption{}\label{T_5ff4d}\tabularnewline
\toprule\noalign{}
...
\caption{}\label{T_f5301}\tabularnewline
\toprule\noalign{}
...
Disease}\label{sec-results-risks}
...
\item
  Irradiate LNL IV only when the upstream level III is pathologically
  confirmed via FNA
\item
  Exclude LNLs I and VII for most patients. I.e., only treat these LNLs
  when they appear clinically metastatic
\item
  Exclude LNL V in all but the most extreme cases. I.e., for advanced
  T-category tumors with midline extension, where all ipsilateral LNLs
  and the contralateral LNLs II to IV present with clinical involvement
  and the upstream involvement is additionally verified by FNA.
\end{itemize}

In essence, this is a summary of the results in
section~\ref{sec-results-risks}. However, it should be considered in
...
For some patterns of involvement the difference in the observed
prevalence for early vs advanced T-category is greater, for other
smaller, than what our model predicts. This is because only one
parameter, \(p_\text{adv.}\), governs this spread by continuing to
evolve the system a certain number of time-steps, while the speed of
spread, here the \(b_v\) parameters, remains constant. However, it is
feasible that these parameters themselves may change over time as the
tumor grows into regions with different lymph drainage pathways.

\subsubsection{Sensitivity and
Specificity}\label{sensitivity-and-specificity}

As hinted at in section~\ref{sec-data-consensus}, the literature values
for sensitivity and specificity pairs of the diagnostic modalities do
not plausibly match some of the observed data. We therefore assumed the
consensus across diagnostic modalities and pathology to represent the
ground truth. This simplification allowed us to investigate the models
general capability to describe a realistic, if not necessarily entirely
accurate dataset. But in the future, this may need to be approached more
rigorously, e.g.~by differentiating between pathologically confirmed and
clinically diagnosed involvement, as the latter is - by definition -
incapable of detecting microscopic disease.
...
subsite for its training and prediction. We have previously speculated
that a mixture model could be a promising approach to capture the
subsite specific spread patterns \citep{zora231470}.
...
\caption{}\label{T_474bc}\tabularnewline
\toprule\noalign{}
